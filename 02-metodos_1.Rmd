...
### Paramétrico {#parametrico-2}

#### Medias {#medias-parametrica-2}

##### Caso Normal y varianza conocida (una cola)
...
Por un de los teoremas de las probabilidades sabemos que $P(A^c) = 1 - P(A)$ y sabemos que si la media de $\mu_0 => \bar{X}~NORMAL(\mu_0, \sigma^2/n)$ entonces podemos asumir $Z = \frac{\bar{X} - \mu_0}{\sigma/\sqrt{n}}$:
...
Como ejemplo, supongamos tenemos una muestra de 20 muestras de sangre y queremos comparar la concentración de linfositos para saber si son distintos al promedio de la población. Supongamos queremos hacerlo al 5%.
...
mu <- 3 # nanolitros nL
sd0 <- .25
xs <- rnorm(20, mu+sd0/2, sd0)
alfa <- .05

Z0 = abs(mean(xs) - mu)/(sd0/length(xs)^.5)
Z0 >= qnorm(1 - alfa/2)
...
##### Caso no normal y varianza conocida (una cola)
...
#### Varianza {#varianza-parametrica-2}
...
La idea de usar $n > 30$ es la sigueinte, pero no funciona parece...

```{r histo-1, tidy=FALSE}
population <- rnorm(30)
hist(unlist(lapply(1:30, FUN = function(x) mean(sample(population, 2)))), 10,
     main="", xlab="Mean")
```
