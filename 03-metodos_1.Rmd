...
Visualmente, se puede ver parte de la formulación en la figura \@ref(fig:normal-3).

```{r normal-3, fig.cap = "Explicación visual de las áreas de rechazo (en rojo) y las áreas de aceptación (sector blanco bajo la curva) para una distribución normal", echo=FALSE, fig.width = 4.5, fig.asp = 0.7, fig.align = 'center'}
library(latex2exp)
opar <- par()$mar
par(mar=c(3,1,1,1))
...
En todos los casos se asume que existe una distribución normal. Es importante resaltar que como en este caso se utiliza la distribución chi cuadrada ($\chi^2$) de características asimétricas.
