...
Dado un espacio de probabilidad $(\Omega, \Im, P)$, se considera una **función de distribución** de la variable aleatoria $X$ ($F_X(x)$) para todos los valores $x$ reales, mediante la ecuación \@ref(eq:funcion-distribucion). En inglés, se denomina **Cumulative Distribution Function**, o CDF.
...
5. $\forall \text{ } x_0, \lim_{x \rightarrow x_0^+} = F_X(x_0)$ (continuidad por la derecha, es decir cuando va de $+\infty$ a $-\infty$)
6. $F_X(x)$ tiene una cantidad finita o numerable de puntos discontinuos

Una variable aleatoria es discreta, si $F_X(x)$ es una función discreta de $x$.

Una variable aleatoria es continua, si $F_X(x)$ es una función continua de $x$.

Las variables aleatorias $X$ e $Y$ son idénticamente distribuidas, si $F_X(x) = F_Y(y), \text{ } \forall x$. Es importante tener claro que eso no quiere decir que $X=Y$ o que $X \neq Y$.

La función que indica la probabilidad de ocurrencia de un *"punto"* en particular, se denomina **Función de Probabilidad de Masa** (**Probability Mass Function**, PMF) y **Función de Densidad de Probabilidad** (**Probability Density Function**, PDF), para el caso discreto y continuo respectivamente. En el caso continuo, el punto tiene probabilidad 0, por lo que en realidad solo está estipulado para entenderlo de manera conceptual.

La PMF se escribe como $f_X(x)$, para diferenciarla de la PDF $F_X(x)$. Ambas están definidas en la ecuación \@ref(eq:PMF-def) y \@ref(eq:PDF-def).
...
$f_X(x)$ es una PDF/PMF de una variable aleatoria $x$, $\Leftrightarrow$

1. $f_X(x) \ge 0, \text{ } \forall x$
2. $\sum_{x} f_X(x) = 1$ o $\int_{-\infty}^{+\infty} f_X(x) dx = 1$


### Distribución de una Función de Distribución de una Variable Aleatoria

Para el caso discreto y continuo respectivamente, se tiene: 

\begin{equation}
CDF: P_Y(Y \in A) = P_Y(Y=y) = \sum_{x \in g^{-1}(y)}P_X(X=x) = \sum_{x \in g^{-1}(y)} f_X(x)
(\#eq:CDF-dist)
\end{equation}

\begin{equation}
PDF: F_Y(y) = P_Y(Y \leq y) = P_X(\{x \in X: g(x) \leq y\}) = P_X(X \in g^{-1}(A))= \int_{x \in X:g(x) \leq y} f_X(x)dx
(\#eq:PDF-dist)
\end{equation}


Una propiedad, es que sea $X$ una variable aleatoria con CDF, función de distribución, $F_X(x)$, con $Y=g(X)$, $x_s = \{x \in X:f_X(x) > 0\}$ y $y_s = \{\text{y} \in y : \text{y}=g (x) \text{ para algún } x \in x_s\}$, entonces:

1. Si $g$ es función creciente en $x_s$, $F_Y(\text{y}) = F_X(g^{-1}(\text{y}))$, para $\text{y} \in y$
2. Si $g$ es función decreciente en $x_s$, $F_Y(\text{y}) = 1 - F_X(g^{-1}(\text{y}))$, siendo $X$ continua, para $\text{y} \in y$


Otra propiedad, es que sea $X$ una variable aleatoria con PDF $f_X(x)$ y sea $Y=g(x)$, donde $g$ es una función monótona, suponga que $f_X(x)$ es continua en $X_c$ y que $g^{-1}(\text{y})$ tiene derivada continua en $y_c$, entonces la PDF de $Y$ está dada por la ecuación \@ref(eq:PDF-monotona)

\begin{equation}
f_Y(\text{y}) =\left\{\begin{matrix}
f_X(g^{-1}(\text{y}))|\frac{d}{d_y}g^{-1}(\text{y})| &, \text{y} \in y_s\\ 
0 & \text{e.o.c} 
\end{matrix}\right.
(\#eq:PDF-monotona)
\end{equation}
