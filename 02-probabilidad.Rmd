...
### Distribución de una Función de Distribución
...
Sea $X$ una variable aleatoria con CDF, función de distribución, $F_X(x)$, con $Y=g(X)$, $x_s = \{x \in X:f_X(x) > 0\}$ y $y_s = \{y \in y_s : y =g (x) \text{ para algún } x \in x_s\}$, entonces:

1. Si $g$ es función creciente en $x_s$, $F_Y(y) = F_X(g^{-1}(y))$, para $y \in Y$
2. Si $g$ es función decreciente en $x_s$, $F_Y(y) = F_X(g^{-1}(y))$
